...
const Meeting = () => {
  ...
  const handleCreateMeeting = async () => {
    ...
    try {
      ...
      if (fetchResp.data.length > 0 && dealData != null) {
        ...
        if (updateResponse?.data?.[0]?.code !== "SUCCESS") {
          console.error("Error updating record:", updateResponse);
        }
      } else {
  ...
  useEffect(() => {
    ZOHO.embeddedApp.on("PageLoad", function (data) {
      ...
      ZOHO.CRM.UI.Resize({ height: "80%", width: "40%" });
    });
